...
StopsTransformer.prototype._transform = function (data, encoding, done) {
  ...
  this.push(triple);
  this.push({ subject: subject, predicate: "http://www.w3.org/1999/02/22-rdf-syntax-ns#type", object: "http://vocab.gtfs.org#Stop"});
  // gtfs:code triple
  if (data["stop_code"] ) {
  ...
  if (data["location_type"] ) {
    this.push({ subject: subject, predicate: "http://vocab.gtfs.org#locationType", object :'"' +  data["location_type"]+'"' });
  }
  // gtfs:parentStation triple
  if (data["parent_station"] ) {
    this.push({ subject: subject, predicate: "http://vocab.gtfs.org#parentStation", object :'"' +  data["location_type"]+'"' });
  }
  done();
};
