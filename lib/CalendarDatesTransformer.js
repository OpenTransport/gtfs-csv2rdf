...
CalendarDatesTransformer.prototype._transform = function (data, encoding, done) {
  ...
  var servicerule = subject + "/" + data["date"];
  ...
  if (data["date"]){
    this.push({ subject: servicerule, predicate: "http://purl.org/dc/terms/date", object: '"' + data["date"] + '"^^http://www.w3.org/2001/XMLSchema#date'});
  }
