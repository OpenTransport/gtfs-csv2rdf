...
TripsTransformer.prototype._transform = function (data, encoding, done) {
  var subject = "http://data.gtfs.org/trips/" + this._feedname + "/" + data["trip_id"];
  ...
  if (data["route_id"]) {
    this.push({ subject: subject, predicate: "http://vocab.gtfs.org/terms#route", object: "http://data.gtfs.org/routes/" + this._feedname + "/" + data["route_id"]});
  }
  if (data["service_id"]) {
    this.push({ subject: subject, predicate: "http://vocab.gtfs.org/terms#service", object: "http://data.gtfs.org/services/" + this._feedname + "/" + data["service_id"]});
  }
