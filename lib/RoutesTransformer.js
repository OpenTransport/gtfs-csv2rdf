...
RoutesTransformer.prototype._transform = function (data, encoding, done) {
  var subject = "http://data.gtfs.org/routes/" + this._options.feedname + "/" + this._options.version + "/" + data["route_id"];
  this.push({ subject: subject, predicate: "http://www.w3.org/1999/02/22-rdf-syntax-ns#type", object: "http://vocab.gtfs.org/terms#Route"});
  if (data["agency_id"]) {
    this.push({ subject: subject, predicate: "http://vocab.gtfs.org/terms#agency", object: "http://data.gtfs.org/agencies/" + this._options.feedname + "/" + this._options.version + "/" + data["agency_id"]});
  }
