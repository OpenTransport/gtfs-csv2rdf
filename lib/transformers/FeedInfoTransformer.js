...
FeedInfoTransformer.prototype._transform = function (data, encoding, done) {
  ...
  if (data["feed_start_date"] && data["feed_end_date"]) {
    ...
    this.push({ subject: temporal, predicate: "http://schema.org/startDate", object: '"' + data["feed_start_date"] + '"^^http://www.w3.org/2001/XMLSchema#date'});
    this.push({ subject: temporal, predicate: "http://schema.org/endDate", object: '"' + data["feed_end_date"] + '"^^http://www.w3.org/2001/XMLSchema#date'});
  }
