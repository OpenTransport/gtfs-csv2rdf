...
util.inherits(TriplesToJSONLD, Transform);
...
TriplesToJSONLD.prototype._flush = function (done) {
  this._createObject(this._jsonld, done);
};

TriplesToJSONLD.prototype._createObject = function (jsonldGraph, callback) {
  //Let's couple @id here
  jsonldGraph = this._coupleJSONLD(jsonldGraph);
  jsonld.compact(jsonldGraph, this["@context"], function(err, compacted) {
    delete(compacted["@context"]);
    callback(err, compacted);//this._jsonld));
  });
}

TriplesToJSONLD.prototype._coupleJSONLD = function (jsonldGraph) {
  //check whether the object already exists in the jsonld somewhere
  var temp = {};
  for (var i = 0; i < jsonldGraph["@graph"].length; i++) {
    if (typeof temp[jsonldGraph["@graph"][i]["@id"]] === "undefined") {
      ...
      //merge the elements: it will contain an @type, or a specific predicate, as we have 1 array element per 1 triple
      for (var key in jsonldGraph["@graph"][i]) {
        //If the key already exists, and if it's not the same, we need to add it in an array.
...
TriplesToJSONLD.prototype._transform = function (triple, encoding, done) {
  ...
  if (N3.Util.isIRI(triple.object)) {
    if (triple.predicate === 'http://www.w3.org/1999/02/22-rdf-syntax-ns#type' || triple.predicate === 'a') {
      ...
      //This URI might need more information... Probably we'll find it later and might add it to this JSON object
